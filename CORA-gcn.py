...
class GCN(torch.nn.Module):
    ...
    def update_edge_values(self, data, G):
        for edge in G.edges():
            node1, node2 = edge
...
# Function to update edge values with node features
def update_edge_values_with_node_features(G, data):
    for edge in G.edges():
        node1, node2 = edge
        edge_value = torch.sum(data.x[node1]) + torch.sum(data.x[node2])
        G[node1][node2]['weight'] = edge_value.item()
...
#TODO: is this correct? edges should be updated during training not before, on the data
noisy_data.x    = update_edge_values_with_node_features(G, noisy_data)
...
def train_model_with_history(data, model, history_dict, history_key, epochs = 200):
    ...
    for epoch in range(epochs):
        ...
        loss.backward()
        optimizer.step()
        # Update edge values with node features
        model.update_edge_values(data, G)
...
original_model = GCN(dataset.num_node_features, dataset.num_classes).to(device)
noisy_model = GCN(dataset.num_node_features, dataset.num_classes).to(device)
